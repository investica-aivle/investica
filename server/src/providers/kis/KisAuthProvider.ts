...
export class KisAuthProvider {
  ...
  public async authenticate(
    ...
    try {
      ...
      this.logger.debug(
        `Sending OAuth request to: ${KisConstants.VIRTUAL_DOMAIN}/oauth2/tokenP`,
      );

      const response = await fetch(
        `${KisConstants.VIRTUAL_DOMAIN}/oauth2/tokenP`,
        {
          method: "POST",
          headers: {
      ...
      this.logger.debug(
        `KIS OAuth response status: ${response.status} ${response.statusText}`,
      );
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `KIS OAuth failed: ${response.status} ${response.statusText}`,
        );
      }
      ...
      tokenExpiresAt.setSeconds(
        tokenExpiresAt.getSeconds() + authResponse.expires_in,
      );

      this.logger.log(
        `KIS authentication successful for account: ${maskedAccountNumber}, token expires at: ${tokenExpiresAt.toISOString()}`,
      );
      ...
      if (
        ...
        error.message.includes("KIS OAuth failed")
      ) {
      ...
      this.logger.error(
        `KIS authentication error for account: ${maskedAccountNumber}`,
        JSON.stringify(
          {
            error: errorMessage,
            stack: errorStack,
  ...
  public parseAccountNumber(accountNumber: string): IAccountNumberParts {
    ...
    if (parts.length !== 2) {
      throw new Error(
        `잘못된 계좌번호 형식입니다: ${accountNumber}. 올바른 형식: XXXXXXXX-XX`,
      );
    }
    ...
    if (CANO.length !== 8 || !/^\d{8}$/.test(CANO)) {
      throw new Error(
        `잘못된 계좌번호 형식입니다: ${CANO}. 계좌번호는 정확히 8자리 숫자여야 합니다.`,
      );
    }
    ...
    if (ACNT_PRDT_CD.length !== 2 || !/^\d{2}$/.test(ACNT_PRDT_CD)) {
      throw new Error(
        `잘못된 계좌상품코드 형식입니다: ${ACNT_PRDT_CD}. 계좌상품코드는 정확히 2자리 숫자여야 합니다.`,
      );
    }
