...
import { IKisSessionData, KisAuthProvider } from "./KisAuthProvider";
...
export class KisTradingProvider {
  ...
  public async executeStockOrder(
    ...
    orderRequest: IStockOrderRequest
  ): Promise<IStockOrderResponse> {
    const maskedAccountNumber = sessionData.accountNumber.replace(/(\d{4})\d+(\d{2})/, '$1****$2');

    // TR_ID 결정 (매수/매도에 따라)
    const trId = orderRequest.orderType === "buy"
      ? KisConstants.TR_ID.STOCK_ORDER.VIRTUAL.BUY
      : KisConstants.TR_ID.STOCK_ORDER.VIRTUAL.SELL;

    this.logger.log(`Executing ${orderRequest.orderType} order for ${orderRequest.stockCode}, quantity: ${orderRequest.quantity}, account: ${maskedAccountNumber}`);

    try {
      // 토큰 만료 체크 및 갱신
      const validSessionData = await this.kisAuthProvider.refreshTokenIfNeeded(sessionData);

      // 계좌번호 분리 (KisAuthProvider 활용)
      const { CANO, ACNT_PRDT_CD } = this.kisAuthProvider.parseAccountNumber(validSessionData.accountNumber);

      // 주문구분 설정 (00:지정가, 01:시장가)
      const orderDivision = orderRequest.orderCondition === "limit"
      ...
      const headers = this.kisAuthProvider.createApiHeaders(validSessionData, trId);
      ...
      // KIS API 호출
      const response = await fetch(`${KisConstants.VIRTUAL_DOMAIN}/uapi/domestic-stock/v1/trading/order-cash`, {
        method: "POST",
      ...
      if (apiResponse.rt_cd === "0") {
        ...
        const orderId = outputData?.ODNO || outputData?.KRX_FWDG_ORD_ORGNO || "UNKNOWN";
        ...
        this.logger.error(
          ...
          JSON.stringify({
            ...
            requestBody: {
              ...
              ORD_QTY: orderRequest.quantity.toString(),
              ORD_UNPR: orderRequest.price ? orderRequest.price.toString() : "0",
              SLL_TYPE: orderRequest.orderType === "sell" ? KisConstants.SELL_TYPE.NORMAL : undefined
            },
            headers: {
              tr_id: trId,
