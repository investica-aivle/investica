...
export class KisBalanceProvider {
     ...
     * 캐시 키 생성 (계좌번호 기반)
     */
    private getCacheKey(accountNumber: string): string {
        return `balance_${accountNumber}`;
    }

    /**
     * 캐시에서 데이터 가져오기
     */
    private getCachedBalance(accountNumber: string): any | null {
        const cacheKey = this.getCacheKey(accountNumber);
        ...
        if (isExpired) {
            ...
            this.logger.debug(`Cache expired for account: ${accountNumber.replace(/(\d{4})\d+(\d{2})/, "$1****$2")}`);
            return null;
        }

        this.logger.debug(`Cache hit for account: ${accountNumber.replace(/(\d{4})\d+(\d{2})/, "$1****$2")}`);
        return cached.data;
    }
     ...
     * 캐시에 데이터 저장
     */
    private setCachedBalance(accountNumber: string, data: any): void {
        const cacheKey = this.getCacheKey(accountNumber);
        this.balanceCache.set(cacheKey, {
            data,
            timestamp: Date.now()
        });
        this.logger.debug(`Cached balance for account: ${accountNumber.replace(/(\d{4})\d+(\d{2})/, "$1****$2")}`);
    }
    ...
    private async fetchBalance(sessionData: IKisSessionData): Promise<any> {
        // 캐시 확인
        const cachedData = this.getCachedBalance(sessionData.accountNumber);
        if (cachedData) {
            return cachedData;
        }

        // 캐시 미스 - 실제 API 호출
        this.logger.debug(`Cache miss - fetching from KIS API for account: ${sessionData.accountNumber.replace(/(\d{4})\d+(\d{2})/, "$1****$2")}`);
        ...
        // 캐시에 저장
        this.setCachedBalance(sessionData.accountNumber, data);
    ...
    async getStockBalances(
        ...
        try {
            ...
            return {
                rt_cd: balanceData.rt_cd || "0",
                msg_cd: balanceData.msg_cd || "",
                msg1: balanceData.msg1 || "",
                output1: holdings,
                message: holdings.length > 0 ? `${holdings.length}개 종목을 보유중이에요` : "보유 주식이 없어요"
            };
        } catch (error) {
            const errorMessage =
                error instanceof Error ? error.message : String(error);
            this.logger.error(`Stock holdings inquiry failed: ${errorMessage}`);
            throw new Error(`보유 주식 목록 조회 실패: ${errorMessage}`);
        }
    }
}
